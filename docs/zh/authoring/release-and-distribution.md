...
Agent App release 应该不可变、可审查、可回滚。Registry 可以分发 package 和授权租户，但本地宿主仍负责校验、readiness、policy 和 runtime 执行。v0.8 还新增一个发布决策：同一个 package 可以面向 Lime 内安装、独立品牌安装、runtime-backed 安装或兼容 Web Host 分发。
...
| `install` | 来自 `app.install.yaml` 的 v0.8 安装模式和 Runtime 关系。 |
