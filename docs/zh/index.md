...
hero:
  name: Agent App
  text: 可执行的完整智能应用。
  tagline: "v0.8 让完整智能应用既能安装到 Lime，也能成为独立品牌 App，或复用系统 Lime Runtime。"
  actions:
    - theme: brand
      text: 阅读规范
...
features:
  ...
  - title: 独立安装
    details: "v0.8 新增 app.install.yaml，让 App 声明 Lime 内安装、独立安装、Runtime-backed 和 Web Host 模式。"
  - title: Capability SDK
    details: "App 通过 lime.ui、lime.storage、lime.agent、lime.connectors、lime.artifacts、lime.evidence 等稳定能力调用 Lime，不依赖内部实现。"
  - title: 组合已有标准
    details: "Agent App 引用 Runtime、UI、Context、Knowledge、Skills、Tools / Connectors、Artifacts、Evidence、Policy、QC 和 Evals，不重新定义它们。"
  - title: 小程序平台心智
...
v0.8 App 应该在执行前可理解，不修改宿主 Core 也能安装，通过 typed capability handles 运行，通过稳定 Agent task 事件观测，能说明交付边界，并能说明用户如何安装它。v0.8 保留 v0.5 分层 manifest、v0.6 `app.runtime.yaml`、v0.7 `app.requirements.yaml` / `app.boundary.yaml` / `app.integrations.yaml` / `app.operations.yaml`，新增 `app.install.yaml` 表达 Lime 内安装、独立安装、Runtime-backed 和 Web Host 打包模式。如果一个 package 不能说明 entry、权限、数据边界、runtime assets、任务输出契约、外部集成、副作用、人工审批边界和安装模式，它还不适合作为 Agent App 分发。
...
2. 写清 v0.7 requirements、boundary、integrations、operations。
3. 写清 v0.8 install contract，说明 Lime 内安装、独立安装、Runtime-backed 或 Web Host 分发方式。
4. 声明 capabilities、storage、Runtime、UI、Context、Knowledge、Skills、Tools / Connectors、Artifacts、Evidence、Policy、QC、permissions 和 Evals。
5. 增加 runtime package assets，并通过 Capability SDK 调宿主能力。
6. 先做 projection 和 readiness，再开放执行。
7. 发布时带 package hashes、compatibility metadata、overlays、install metadata 和 rollback guidance。
