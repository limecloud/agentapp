...
hero:
  name: Agent App
  text: 可执行的完整智能应用。
  tagline: "v0.9 让完整智能应用通过宿主能力和 App Server bridge 契约运行。"
  actions:
    - theme: brand
      text: 阅读规范
...
features:
  ...
  - title: 独立安装
    details: "v0.9 保留 app.install.yaml，并补充宿主中介的 App Server bridge 执行映射。"
...
v0.9 App 应该在执行前可理解，不修改宿主 Core 也能安装，通过 typed capability handles 运行，通过 RuntimeCore 派生的 Agent task 事件观测，能说明交付边界，能说明用户如何安装它，并能说明 `lime.agent` / `lime.workflow` 调用如何进入 App Server JSON-RPC。v0.9 保留 v0.5 分层 manifest、v0.6 `app.runtime.yaml`、v0.7 `app.requirements.yaml` / `app.boundary.yaml` / `app.integrations.yaml` / `app.operations.yaml` 和 v0.8 `app.install.yaml`，新增 `agentRuntime.bridge` 表达 App Server bridge 映射。如果一个 package 不能说明 entry、权限、数据边界、runtime assets、任务输出契约、外部集成、副作用、人工审批边界、安装模式和 runtime bridge，它还不适合作为 Agent App 分发。
...
4. 写清 v0.9 App Server bridge profile，说明 `lime.agent` / `lime.workflow` 如何执行。
