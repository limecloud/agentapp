...
- `new`、`resume`、`continue`、`fork` session modes
...
- [`app.requirements.yaml`](../../examples/content-factory-app/app.requirements.yaml)：声明 MVP 需求、非目标、后续阶段和验收标准
- [`app.boundary.yaml`](../../examples/content-factory-app/app.boundary.yaml)：声明 App / Host / Cloud / connector / external system / human 职责边界
- [`app.integrations.yaml`](../../examples/content-factory-app/app.integrations.yaml)：声明由 Host/Cloud 托管的文件、外部规划表和 CLI adapter 需求
- [`app.operations.yaml`](../../examples/content-factory-app/app.operations.yaml)：声明副作用、审批、dry-run、幂等和 evidence 规则
