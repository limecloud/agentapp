---
title: 规范
description: Agent App v0.9 可执行应用包、Capability SDK 契约、Agent task runtime control plane、需求边界、安装模式与 App Server bridge profile。
---
...
v0.9 继承 v0.6 的 `lime.agent` task runtime control plane、v0.7 **Requirement Boundary & Capability Handoff** 与 v0.8 **Standalone Installation & Runtime Separation**，并新增 **App Server Bridge Profile**：App 必须能把业务需求拆成 App、Host、Cloud、Connector、外部系统和人工决策的责任边界，同时声明它是安装到 Lime Desktop、作为独立品牌 App、复用系统 Lime Runtime，还是运行在兼容 Web Host 中，并声明 App 面 Agent task 如何映射到 Desktop Host IPC、App Server JSON-RPC、RuntimeCore services 和 execution backend。
...
├── app.runtime.yaml          # v0.6 可选：lime.agent task runtime control plane；v0.9 可选：App Server bridge profile
├── app.requirements.yaml     # v0.7 可选：需求项、MVP、非目标、验收标准
├── app.boundary.yaml         # v0.7 可选：App / Host / Cloud / Connector / External / Human 边界
├── app.integrations.yaml     # v0.7 可选：Host/Cloud 托管的外部集成需求
├── app.operations.yaml       # v0.7 可选：操作副作用、审批、dry-run、evidence
├── app.install.yaml          # v0.8 可选：Lime 内安装、独立安装、runtime-backed 安装模式
├── dist/
...
v0.9 保留分层配置原则：`APP.md` 只承载发现元数据和人类可读指南，复杂配置（capabilities、entries、permissions、errors、i18n、signature、Agent task runtime contracts、App Server bridge profile、需求边界、外部集成、操作副作用和安装模式）按需放入独立 `app.*.yaml` 文件，避免 frontmatter 膨胀。
...
| 字段 | 用途 |
| --- | --- |
| `manifestVersion` | manifest 协议版本，当前版本应填 `0.9.0`。 |
...
| `agentRuntime` | v0.6 Agent task runtime control plane 简写；详细配置优先使用 `app.runtime.yaml`。 |
| `requirements` | v0.7 需求、MVP、非目标和验收标准简写；详细配置优先使用 `app.requirements.yaml`。 |
| `boundary` | v0.7 App / Host / Cloud / Connector / External / Human 职责边界简写；详细配置优先使用 `app.boundary.yaml`。 |
| `integrations` | v0.7 外部集成需求简写；详细配置优先使用 `app.integrations.yaml`。 |
| `operations` | v0.7 操作副作用、审批、dry-run 和 evidence 简写；详细配置优先使用 `app.operations.yaml`。 |
| `install` | v0.8 Lime 内安装、独立安装、Runtime-backed 和 Web Host 安装模式简写；详细配置优先使用 `app.install.yaml`。 |
...
| `app.runtime.yaml` | v0.6 Agent task 事件 / 结果、结构化输出、审批、session、工具发现、checkpoint、可观测性；v0.9 App Server bridge profile | manifest 自动加载 |
| `app.requirements.yaml` | v0.7 需求项、MVP 范围、非目标、后续阶段和验收标准 | manifest 自动加载 |
| `app.boundary.yaml` | v0.7 App / Host / Cloud / Connector / External / Human 职责边界 | manifest 自动加载 |
| `app.integrations.yaml` | v0.7 Host/Cloud 托管的外部系统、CLI、MCP、API、webhook 或 browser adapter 需求 | manifest 自动加载 |
| `app.operations.yaml` | v0.7 操作副作用、审批、dry-run、幂等和 evidence 要求 | manifest 自动加载 |
| `app.install.yaml` | v0.8 Lime 内安装、独立安装、Runtime-backed 和 Web Host 安装模式 | manifest 自动加载 |
...
`app.install.yaml` 是 v0.8 中产品打包和 Runtime 底座之间的边界。它允许同一个业务包以多种形态分发，而不改变 App 内部业务实现：
...
v0.6 不把 Agent App 变成新的 AgentRuntime，也不要求 App 直接集成某个模型 SDK。它把 `lime.agent` 的任务运行控制面标准化，让 App、Host、AgentRuntime、ToolHub、Evidence 和 UI 对同一个 task 有一致的事实源。
...
v0.6 建议需要 `lime.agent` 的 App 增加 `app.runtime.yaml`，集中声明 Agent task runtime 约束：
...
`agentRuntime.bridge` 是 v0.9 的 App Server bridge profile。它声明宿主应如何把 App 面的 `lime.agent` / `lime.workflow` 调用投影到后端事实源，但它不是 App 可以直接访问的网络地址或进程句柄。
...
`expectedOutput` 可以继续放在 `lime.agent.startTask` 请求里，但 v0.6 推荐使用显式 JSON Schema：
...
v0.6 把工具审批、用户提问和补上下文统一为 runtime approval：
...
v0.7 不替代 v0.6 runtime control plane，而是在它之上补齐“需求边界”和“能力交接”。给定一份脱敏业务需求，Agent App 必须能说明：哪些由 App 做成用户可见体验，哪些由 Host 执行，哪些由 Cloud 治理，哪些需要 Connector 适配，哪些事实仍留在外部系统，哪些必须由人确认。
...
readiness:
  required:
    - check: sdk_version
      expect: ">=0.9.0"
      blocker: true
      message: 需要 Lime SDK v0.9.0 或更高版本
