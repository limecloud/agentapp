---
title: 规范
description: Agent App v0.6 可执行应用包、Capability SDK 契约和 Agent task runtime control plane。
---
...
v0.6 保留 v0.5 从 [Agent Skills 标准](https://agentskills.io) 借鉴的发现与编写纪律：`APP.md` frontmatter 保持精简，详细配置下沉到独立文件；同时新增 `app.runtime.yaml` 表达 `lime.agent` 的 task runtime control plane，包括事件 / 结果信封、结构化输出、runtime approval、session resume / fork、工具发现、checkpoint scope 与可观测性。
...
v0.6 保留分层配置原则：`APP.md` 只承载发现元数据和人类可读指南，复杂配置（capabilities、entries、permissions、errors、i18n、signature、Agent task runtime contracts）按需放入独立 `app.*.yaml` 文件，避免 frontmatter 膨胀。
...
| 字段 | 用途 |
| --- | --- |
| `manifestVersion` | manifest 协议版本，v0.6 应填 `0.6.0`。 |
...
readiness:
  required:
    - check: sdk_version
      expect: ">=0.6.0"
      blocker: true
      message: 需要 Lime SDK v0.6.0 或更高版本
