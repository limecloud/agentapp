---
title: 规范
description: Agent App v0.7 可执行应用包、Capability SDK 契约、Agent task runtime control plane 与需求边界。
---
...
v0.7 继承 v0.6 的 `lime.agent` task runtime control plane，并新增 **Requirement Boundary & Capability Handoff**：App 必须能把业务需求拆成 App、Host、Cloud、Connector、外部系统和人工决策的责任边界，避免把客户私有系统、厂商适配或云服务职责硬塞进 App 包。
...
v0.7 保留分层配置原则：`APP.md` 只承载发现元数据和人类可读指南，复杂配置（capabilities、entries、permissions、errors、i18n、signature、Agent task runtime contracts、需求边界、外部集成和操作副作用）按需放入独立 `app.*.yaml` 文件，避免 frontmatter 膨胀。
...
| 字段 | 用途 |
| --- | --- |
| `manifestVersion` | manifest 协议版本，当前版本应填 `0.7.0`。 |
...
readiness:
  required:
    - check: sdk_version
      expect: ">=0.7.0"
      blocker: true
      message: 需要 Lime SDK v0.7.0 或更高版本
